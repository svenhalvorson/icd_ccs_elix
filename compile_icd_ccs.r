...
# I had originally programmed this in python so some of the ways
# I coded this are a bit strange seeming. Lots more of explicit loops.

# Set up ------------------------------------------------------------------

# Going to set up my libraries and the file names to assemble the dataset
...
base_dir = 'C:/Users/HALVORS/Documents/icd_list/'
...
# CCSR codes:
ccsr10_cm_p = 'DXCCSR2020_1/$DXCCSR2020.CSV'
...
icd9_cm = read_excel(
  paste0(base_dir, icd9_cm_p)
)
icd9_pcs = read_excel(
  paste0(base_dir, icd9_pcs_p)
)
...
icd10_cm = read_fwf(
  paste0(base_dir, icd10_cm_p),
  col_sizes
)

icd10_pcs = read_fwf(
  paste0(base_dir, icd10_pcs_p),
  col_sizes
)
...
ccs9_cm = read_csv(
  paste0(base_dir, ccs9_cm_multi_p)
)
ccs9_pcs = read_csv(
  paste0(base_dir, ccs9_pcs_multi_p)
)
ccs9_cm['icd_type'] = 'cm'
...
ccs9_single_cm = read_csv(
  paste0(base_dir, ccs9_cm_single_p)
)

ccs9_single_pcs = read_csv(
  paste0(base_dir, ccs9_pcs_single_p),
  col_types = cols_only(
...
ccs10_cm = read_csv(
  paste0(base_dir, ccs10_cm_p)
)
ccs10_pcs = read_csv(
  paste0(base_dir, ccs10_pcs_p)
)
...
elix9 = read_sas(
  paste0(base_dir, elix9_p)
)
elix10 = read_sas(
  paste0(base_dir, elix10_p)
)
...
icd_ccs %>%
  write_tsv(
    paste0(base_dir, 'icd_ccs_elix_', str_remove_all(Sys.Date(), '-'), '.txt')
  )
